...
namespace BorderHit
{
	...
	std::pair<std::vector<SimpleLine2D>,TraveledLine> RectangleHitter::getLinesWithSpeedWithTrailTime(size_t startIndex, double speed, double time,double trailTime) {
		// f.e. 1 line => 2 points / reflections, 2 lines => 3 points , ...
			...
			if (const VerticalLine2D* vertLine = std::get_if<VerticalLine2D>(&line)) { // vertical lines will always stay vertical in a box
				while (backwardsTravelDistance > 0) {
					...
					if (backwardsTravelDistance < overallLength) {
						...
						double signedDistance = backwardsTravelDistance; // decide the direction in which to go
						...
						TraveledLine traveledLine{ currentIndex,backwardsTravelDistance };
		...
		if (const VerticalLine2D* vertLine = std::get_if<VerticalLine2D>(&line)) { // vertical lines will always stay vertical in a box
			...
			while (distanceToTravel > 0) {
				...
				if (distanceToTravel < overallLength) {
					// only travel on the line
					//startPoint.y + distanceToTravel
					double pointYOffsetFrom = distanceToTravel - speed * trailTime;
					//pointYOffsetFrom -= speed * trailTime;
					...
					resultCutLines.push_back({ Position2D{startPoint.x,std::abs(startPoint.y + pointYOffsetFrom)},
						Position2D{startPoint.x,std::abs(startPoint.y + distanceToTravel)} });
					// store the already traveled Distance
					TraveledLine traveledLine{currentIndex,distanceToTravel};
					return std::pair{ resultCutLines, traveledLine};
				}
				else {
					double pointYOffsetFrom = distanceToTravel - speed * trailTime; // with a trail the start can be on the line,this start is calculated here
					// the line is completly in the trail
					resultCutLines.push_back(SimpleLine2D{ Position2D{startPoint.x,std::abs(startPoint.y + pointYOffsetFrom)},
	...
	std::pair<std::vector<SimpleLine2D>, TraveledLine> RectangleHitter::getLinesWithSpeedWithTrailLength(size_t startIndex, double speed, double time, double distance) {
		double timeLength = distance / speed; // v = s/t <=> s = v * t <=> t = s / v
		return this->getLinesWithSpeedWithTrailTime(startIndex, speed, time, timeLength);
	}


	std::vector<SimpleLine2D> SimpleRectangleHitter::linesTrailTime(double speed, double trailTime, double timeOffset) {
	...
	std::vector<SimpleLine2D> SimpleRectangleHitter::linesTrailLength(double speed, double length, double timeOffset) {
		auto [lines, traveledLine] = this->getLinesWithSpeedWithTrailLength(this->currentLineIndex, speed, this->time, length); // always draw a fixed distance if a line gets finished the next lines also have to be drawn
		this->currentLineIndex = traveledLine.lineIndex;
		this->time = (traveledLine.traveledDistance / speed) + timeOffset; // v = s / t <=> s = v * t <=> t = s / v 
		return lines;
	}
